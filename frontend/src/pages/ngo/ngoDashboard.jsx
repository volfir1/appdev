...
import { Table, Button, Badge, Loader, Title, Card, Container, Group, Text, ActionIcon, Tooltip, Stack, Divider, Box, Select } from "@mantine/core";
import { CheckCircle, XCircle, Clock, Users, FileText, Calendar, User } from "lucide-react";


import { fetchReferrals, updateReferral } from "../shared/referrals/referralService";
...
export default function NgoDashboard() {
  ...
  // Removed unused: households, programs, role
  ...
  const handleStatusChange = async (id, status, notes = "") => {
    ...
    try {
      ...
      setReferrals(referrals.map(r => r._id === id ? { ...r, status, notes: status === 'rejected' ? notes : r.notes } : r));
      notificationService.success(
        `Referral ${status.charAt(0).toUpperCase() + status.slice(1)}`,
        status === 'approved' ? 'Referral has been approved successfully' : 'Referral has been cancelled.'
      );
    } catch {
  ...
  const completedCount = filteredReferrals.filter(r => r.status === 'completed').length;
      ...
      <Container size="xl" className="py-8">
          ...
          <Group justify="space-between" className="mb-4">
            <div>
              ...
              <Text size="sm" className="text-gray-600">
                Approve or cancel referral requests. NGOs cannot create new referrals.
              </Text>
            </div>
            <Group gap="lg">
            ...
            >
              <Table.Thead className="bg-gradient-to-r from-rose-100 to-rose-50">
                <Table.Tr>
                  ...
                  <Table.Th className="text-gray-700 font-semibold">BarangRefer Beneficiaryay</Table.Th>
                  <Table.Th className="text-gray-700 font-semibold">
                    <Group gap={6}>
                      <User size={16} />
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        {((ref.status === "approved" || ref.status === "completed") && ref.approvedBy) && (
                          <Text size="xs" className="text-green-700 font-medium">
                            ✓ Approved by: {ref.approvedBy?.name || ref.approvedBy?.email || 'Admin'}
                          </Text>
                        )}
                        {(ref.status === "rejected" || ref.status === "cancelled") && ref.approvedBy && (
                          <Text size="xs" className="text-red-700 font-medium">
                            ✗ Rejected by: {ref.approvedBy?.name || ref.approvedBy?.email || 'Admin'}
                          </Text>
                        )}
                      </div>
                    </Table.Td>
                    <Table.Td style={{ maxWidth: 350, whiteSpace: 'pre-line', wordBreak: 'break-word' }}>
                      <Tooltip 
                      ...
                      >
                        <Text 
                          ...
                          size="sm"
                          style={{ whiteSpace: 'pre-line', wordBreak: 'break-word', maxWidth: 350 }}
                        >
                          {ref.notes || 'No notes'}
                        </Text>
                      </Tooltip>
                        ...
                        {ref.status === "pending" && (
                          <>
                            ...
                            <Tooltip label="Cancel referral">
                              <Button
                                leftSection={actionLoading[ref._id] === 'rejected' ? <Loader size={14} /> : <XCircle size={14} />}
                                size="xs"
                                radius="md"
                                onClick={() => {
        ...
        >
          ...
          <Group mt="md" justify="flex-end">
            <Button variant="default" onClick={() => setCancelModal({ open: false, id: null })}>Cancel</Button>
            <Button color="red"
              onClick={async () => {
                await handleStatusChange(cancelModal.id, "rejected", cancelNotes);
