...
export default function NgoNavbar() {
  ...
  const isActive = (path) => {
    if (path === 'profile') {
      return location.pathname === `/ngo/profile/${userId}`;
    }
    return location.pathname === `/ngo/${path}`;
  };


  const navItems = [
    { path: 'dashboard', label: 'Dashboard', icon: IconUser },
    { path: 'geospatial', label: 'Geospatial', icon: IconUser },
    { path: 'programs', label: 'Programs', icon: IconUser },
    ...
    <>
      <nav className="admin-navbar">
        <div className="navbar-container">
          {/* Logo/Brand */}
          <Link to="/ngo/dashboard" className="navbar-brand">
            ActOnPov NGO
          </Link>
            ...
            <div className="navbar-nav">
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.path}
                    to={item.path === 'profile' ? `/ngo/profile/${userId}` : `/ngo/${item.path}`}
